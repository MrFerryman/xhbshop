...
protocol XHPageViewControllerDelegate: NSObjectProtocol {
    // MARK: 获取当前子控制器的角标
    func xh_MenuPageCurrentSubController( index: NSInteger, menuPageController: XHPageViewController)
}

class XHPageViewController: UIViewController {
    
    // MARK:- 对外提供的属性
    /// 代理
    weak var delegate: XHPageViewControllerDelegate?
    /// 滑块颜色
    ...
    fileprivate var headerLine: UIView {
        let view = UIView()
        view.backgroundColor = XHRgbColorFromHex(rgb: 0xeeeeee)
        return view
    }
...
// MARK:- 更新视图
extension XHPageViewController {
...
extension XHPageViewController {
    ...
    @objc private func subTitleClick(_ btn: UIButton) {
        ...
        if let btnIndex = subTitleBtnArray.index(of: btn) {
            ...
            delegate?.xh_MenuPageCurrentSubController(index: btnIndex, menuPageController: self)
        }
    }
}
...
extension XHPageViewController: UIPageViewControllerDelegate, UIPageViewControllerDataSource {
    /// 前一个控制器
    ...
    func pageViewController(_ pageViewController: UIPageViewController, didFinishAnimating finished: Bool, previousViewControllers: [UIViewController], transitionCompleted completed: Bool) {
        ...
        delegate?.xh_MenuPageCurrentSubController(index: index, menuPageController: self)
