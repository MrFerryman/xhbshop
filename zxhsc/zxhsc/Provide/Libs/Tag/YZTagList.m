...
{
    _tagMargin = 10;
    _tagColor = [UIColor colorWithRed:214/255.0 green:214/255.0 blue:214/255.0 alpha:1];
    _tagButtonMargin = 8;
    _tagButtonMarginHeight = 8;
    _tagCornerRadius = 5;
    _borderWidth = 0.1;
...
- (void)addTag:(NSString *)tagStr
{
    ...
    if (_tagClass == nil) {
        ...
        tagButton.imageW = _tagButtonImageW;
        tagButton.imageH = _tagbuttonImageH;
    }
    ...
    tagButton.layer.shadowOpacity = 0.15;
    tagButton.layer.shadowOffset = CGSizeMake(1.5, 1.5);
//    tagButton.clipsToBounds = YES;
    ...
    [tagButton setImage:_tagSelectedimage forState:UIControlStateSelected];
...
//-(void)selectButton:(ZYThreatreAndMovieModel *)model{
//    
//}
// 拖动标签
...
- (void)selectedTag:(NSString *)tagStr{
    // 获取对应的标题按钮
    YZTagButton *button = self.tags[tagStr];
//    NSLog(@"button.title = %@",button.titleLabel.text);
    ...
    if (button.isSelected == YES) {
        ...
        button.selected = NO;
        [button setBackgroundColor:[UIColor colorWithRed:234/255.0 green:32/255.0 blue:0/255.0 alpha:1]];
        
        [[NSNotificationCenter defaultCenter]postNotificationName:@"cancelselectedBoxOfficeTagButton" object:nil userInfo:@{@"name":tagStr}];
        
    }else{
        
        button.selected = YES;
        [button setBackgroundColor:[UIColor colorWithRed:234/255.0 green:236/255.0 blue:237/255.0 alpha:1]];
        
        [[NSNotificationCenter defaultCenter]postNotificationName:@"selectedBoxOfficeTagButton" object:nil userInfo:@{@"name":tagStr}];
...
//    NSLog(@"btny = %f",btnY);
    ...
    // 获取按钮高度
    CGFloat btnH = extreMargin? titleH + 2 * _tagButtonMarginHeight:tagButton.bounds.size.height;
    if (_tagDeleteimage && extreMargin == YES) {
        CGFloat height = imageViewWH > titleH ? imageViewWH : titleH;
        btnH = height + 2 * _tagButtonMarginHeight;
    }
...
//    CGFloat rightWidth = self.bounds.size.width - btnX;
    ...
    CGFloat rightWidthOfFirst = rightWidth - 120;
   
    if (btnY == 20 &&  rightWidthOfFirst < btnW) {
        
        NSLog(@"_FirstRowWidth = %f",_FirstRowWidth);
        btnX = _tagMargin;
        btnY = CGRectGetMaxY(preButton.frame) + 15;
    }
    ...
    if (rightWidth < btnW) {
     ...
     _FirstRowWidth = btnX + btnW + _tagMargin ;
