...
class XHQRCodeManager: NSObject {
    /// 创建二维码
    func generateQRCodeImage(info: String, size: CGSize, avartarImg: String?) -> UIImage {
        ...
        if avartarImg != nil {
            return insertAvatarImage(qrimage: UIImage(ciImage: outputImage!), avatar: UIImage(named: avartarImg!)!)
        }
    ...
    func barCodeImage(barCodeStr:String,barCodeSize:CGSize) -> UIImage {
        ...
        let resultImage = outputImage?.transformed(by: CGAffineTransform.identity.scaledBy(x: scaleX, y: scaleY))
    ...
    func addText(image:UIImage,textName:String) ->UIImage{
        ...
        barText.draw(in: CGRect.init(x: 0, y: image.size.height-4, width: size.width, height: 30), withAttributes: [NSAttributedStringKey.font:UIFont.systemFont(ofSize: 18.0),NSAttributedStringKey.backgroundColor:UIColor.clear,NSAttributedStringKey.paragraphStyle:textStyle])
